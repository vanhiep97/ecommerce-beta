                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <strong><a href="{{ route('page-create-role') }}" class="btn btn-danger btn-sm">Thêm mới <span class="ti-plus"></span></a></strong>
                        </div>
                        <div class="card-body">
                            <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                                ...
                                @if(!empty($roles))
                                    @foreach($roles as $key => $role)
                                        <tr>
                                            <td>{!! $key + 1 !!}</td>
                                            <td>{!! !empty($role->name) ? $role->name : 'Dữ liệu trống' !!}</td>
                                            <td>{!! !empty($role->label) ? $role->label : 'Dữ liệu trống' !!}</td>
                                            <td>{!! !empty($role->created_at) ? $role->created_at : 'Dữ liệu trống' !!}</td>
                                            <td>{!! !empty($role->updated_at) ? $role->updated_at : 'Dữ liệu trống' !!}</td>
                                            <td>
                                                <div class="input_wrapper">
                                                    <input type="checkbox" data-id="{{$role->id}}" id="toggle-class" class="switch_4" {{ $role->status == 1 ? 'checked' : '' }}>
                                            ...
                                            <td>
                                                <a href="{{ route('page-edit-role', ['id' => $role->id]) }}" class="btn btn-success"><i class="fa fa-edit"></i></a>
                                                <a class="btn btn-warning" onclick="del({{ $role->id }})"><i class="fa fa-trash"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
        ...
        var del = function (id) {
            swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        type:'POST',
                        url:'{{ url("/s-admin/role/delete") }}' +'/' +id,
                        contentType: false,
                        processData: false,
                        data: form_data,
                    }).done(function(rs){
